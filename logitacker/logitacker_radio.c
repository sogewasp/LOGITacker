...
/* Structure describing the state of the radio */
...
void radio_esb_event_handler(nrf_esb_evt_t * p_event)
{
	switch (p_event->evt_id)
	{
		...
		case NRF_ESB_EVENT_RX_RECEIVED:
			...
			if (m_radio_state.channel_hop_enabled && m_radio_state.channel_hop_disable_on_rx)
	...
	if (m_radio_state.event_handler != NULL)
		m_radio_state.event_handler(p_event);
}
...
void timer_channel_hop_event_handler(void* p_context)
{
	if (m_radio_state.channel_hop_enabled)
	{
		...
		/* Restart timer */
		uint32_t err_code = app_timer_start(m_timer_channel_hop, \
			APP_TIMER_TICKS(m_radio_state.channel_hop_delay_ms), NULL);
...
uint32_t logitacker_radio_init(nrf_esb_event_handler_t esb_event_handler,
								 radio_event_handler_t radio_event_handler)
{
	...
	m_radio_state.event_handler			= esb_event_handler;
	...
	// Pass custom ESB event handler with call through
...
uint32_t radio_start_channel_hopping(uint32_t interval, uint32_t start_delay_ms,
										bool disable_on_rx)
{
...
uint32_t radio_enable_rx_timeout_event(uint32_t timeout_ms) {
	...
	if (m_radio_state.rx_timeout_enabled)
		app_timer_stop(m_timer_no_rx_timeout); //stop timer before restart
...
uint32_t logitacker_radio_convert_promiscuous_frame_to_default_frame(
		nrf_esb_payload_t *p_out_payload,
		nrf_esb_payload_t const in_promiscuous_payload)
{

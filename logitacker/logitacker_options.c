...
uint32_t logitacker_options_update_flash(void)
{
	...
	// assure token is zeroed out before usage
	memset(&find_token, 0, sizeof(find_token));

	// only search for file_id-key-combination once (not account for multiple entries)
	res = fds_record_find(LOGITACKER_FLASH_FILE_ID_GLOBAL_OPTIONS, LOGITACKER_FLASH_KEY_GLOBAL_OPTIONS_LOGITACKER, &record_desc, &find_token);
	if (res != FDS_SUCCESS) {
...
uint32_t logitacker_options_store_to_flash(void)
{
	...
	// try to update first
	res = logitacker_options_update_flash();
	if (res == NRF_SUCCESS) {
...
uint32_t logitacker_options_restore_from_flash(void) 
{
	...
	// Copy flash data to global structure

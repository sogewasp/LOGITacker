...
void main_usbd_hid_keyboard_event_handler(app_usbd_class_inst_t const *p_inst,
											app_usbd_hid_user_event_t event)
{
	if (p_processor != NULL && p_processor->p_usb_hid_keyboard_event_handler != NULL)
		// call keyboard event handler function of p_processor
		// and hand in p_processor (self) as first arg
		(*p_processor->p_usb_hid_keyboard_event_handler)(p_processor, p_inst, event);
}

/* ** Timer ** main/master event handler */
void main_event_handler_timer_next_action(void *p_context)
{
	if (p_processor != NULL && p_processor->p_timer_handler != NULL)
		// call timer handler function of p_processor
		// and hand in p_processor (self) as first arg
		(*p_processor->p_timer_handler)(p_processor, p_context);
}

/* ESB (Enhaced ShockBurst) envent handler */
void main_event_handler_esb(nrf_esb_evt_t *p_event)
{
	...
	if (p_processor != NULL && p_processor->p_esb_handler != NULL)
		// call ESB handler function of p_processor
		// and hand in p_processor (self) as first arg
		(*p_processor->p_esb_handler)(p_processor, p_event);
}
...
void main_event_handler_radio(radio_evt_t const *p_event)
{
	...
	if (p_processor != NULL && p_processor->p_radio_handler != NULL)
		// call radio handler function of p_processor
		// and hand in p_processor (self) as first arg
		(*p_processor->p_radio_handler)(p_processor, p_event);
}
...
static void main_event_handler_bsp(bsp_event_t ev)
{
	...
	if (p_processor != NULL && p_processor->p_bsp_handler != NULL)
		// call BSP handler function of p_processor and hand in p_processor (self) as first arg
		(*p_processor->p_bsp_handler)(p_processor, ev);
}

// Transfers execution to active_enum_process_sub_event
void logitacker_enter_mode_passive_enum(uint8_t *rf_address)
{
	if (p_processor != NULL && p_processor->p_deinit_func != NULL)
		(*p_processor->p_deinit_func)(p_processor);
...
void logitacker_enter_mode_pair_sniff(void)
{
	if (p_processor != NULL && p_processor->p_deinit_func != NULL)
		(*p_processor->p_deinit_func)(p_processor);
...
void logitacker_enter_mode_pair_device(uint8_t const *rf_address)
{
	...
	logitacker_pairing_info_t pi =
	{
		...
		.device_caps				=
			LOGITACKER_DEVICE_CAPS_UNIFYING_COMPATIBLE |
			LOGITACKER_DEVICE_CAPS_LINK_ENCRYPTION, // use link encryption, to account for MouseJack patched firmwares
		// use keyboard, for latest receiver firmwares character keys [a-zA-Z] are blacklisted for non-keyboard devices
		.device_type				= LOGITACKER_DEVICE_UNIFYING_TYPE_KEYBOARD,
...
void logitacker_enter_mode_discovery(void)
{
	if (p_processor != NULL && p_processor->p_deinit_func != NULL)
		(*p_processor->p_deinit_func)(p_processor);
...
void logitacker_quit_mode_discovery(void)
{
...
uint32_t logitacker_init()
{
	...
	// Prompt (shoudn't it be later?)
	sprintf(g_logitacker_cli_name, "LOGITacker $ ");

	// Initialize flash memory
	logitacker_flash_init();

	// try to restore options from flash (updates stats like boot count)
	logitacker_options_restore_from_flash();

	// store back updated options
	logitacker_options_store_to_flash();

	// HF clock needed by ESB radio part
	clocks_start();

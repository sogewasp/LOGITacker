...
typedef struct
{
	...
	uint8_t						payload_data[32]; /* maximum of 32 bytes,
	(we ignore capabilities of newer nRF5 series, as it doesn't apply to Logitech devices) */
...
#define LOGITACKER_USB_HID_GENERIC_INTERFACE_ENDPOINT_LIST()    ( LOGITACKER_USB_HID_GENERIC_EPIN )
...
#define LOGITACKER_USB_HID_KEYBOARD_INTERFACE_ENDPOINT_LIST()   ( LOGITACKER_USB_HID_KEYBOARD_EPIN )
...
#define LOGITACKER_USB_HID_MOUSE_INTERFACE_ENDPOINT_LIST()      ( LOGITACKER_USB_HID_MOUSE_EPIN )
...
uint32_t logitacker_usb_write_hidraw_input_report(
			logitacker_mode_t logitacker_mode,
			logitacker_usb_hidraw_report_type_t type,
			size_t length,
			const void * data);
